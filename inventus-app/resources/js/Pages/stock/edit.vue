...
const form = useForm({
    ...
    unit_value: props.stock.unit_value ?? '',
    supplier_id: props.stock.supplier_id ?? '',
    stock_category_id: props.stock.stock_category_id ?? ''
});
...
const submit = () => {
    form.patch(stocks.update.url(props.stock.id), {
        preserveState: true,
        onFinish: () => {
...
<template>
    <Head title="Editar Produto - Inventus +" />
    <div class="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col">
        ...
        <main class="flex-1 max-w-3xl w-full mx-auto px-6 py-8">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h1 class="text-xl font-bold text-white tracking-tight">Editar Produto</h1>
                    <p class="text-xs text-zinc-400 mt-0.5">Atualize os dados de {{ form.name || 'produto' }}</p>
                </div>
                <Link
                    :href="stocks.index()"
